...
def test_clean_zero_findings(clean, clean_emb):
    ...
    assert DuplicateLabelConflict().detect(clean, clean_emb, None) == []
...
def test_duplicates_with_consistent_labels_are_not_conflicts(clean, workdir):
    ...
    assert DuplicateLabelConflict().detect(ds, stub_embeddings(ds), None) == []
...
def test_conflicting_labels_found(clean, workdir):
    ...
    assert conflicted
    fs = DuplicateLabelConflict().detect(ds, stub_embeddings(ds), None)

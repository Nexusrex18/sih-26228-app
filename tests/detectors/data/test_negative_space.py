...
from _triggers import TorchHandle  # noqa: F401  (import check only)
...
def test_fully_annotated_dataset_zero_findings(two_object):
    ...
    assert NegativeSpace().detect(two_object, None, FakeBoxDetector(two_object)) == []
...
def test_withheld_annotations_found_by_contributor_pattern(two_object):
    ...
    fs = NegativeSpace().detect(poisoned, None, FakeBoxDetector(two_object))     # model sees the truth
...
def test_scattered_misses_across_contributors_are_not_a_pattern(two_object):
    ...
    assert NegativeSpace().detect(ds, None, FakeBoxDetector(two_object)) == []

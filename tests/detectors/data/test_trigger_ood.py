...
def test_clean_zero_findings_model_free(clean):
    assert TriggerArtifact().detect(clean, None, None) == []


def test_frequency_residue_finds_the_trigger_and_names_where(poisoned):
    ds, trig = poisoned
    fs = TriggerArtifact().detect(ds, None, None)
...
def test_isolated_anomalies_do_not_count(clean, workdir):
    ...
    assert TriggerArtifact().detect(ds, None, None) == []          # 2 < max(5, 1%)


# ------------------------------------------------------------------ (d) patch saliency
def test_patch_saliency_corroborates_and_raises_severity(poisoned, poisoned_model):
    ds, trig = poisoned
    fs = TriggerArtifact().detect(ds, None, poisoned_model)
...
def test_clean_dataset_clean_model_zero_findings(clean, clean_model):
    assert TriggerArtifact().detect(clean, None, clean_model) == []


def test_sweep_on_named_candidates_finds_region_without_frequency_residue(poisoned, poisoned_model):
    ds, trig = poisoned
    cand = sorted(trig)[:12]
    det = TriggerArtifact({"z_thr": 1e9}, candidate_samples=cand)     # (a) disabled -> only the sweep
    fs = det.detect(ds, None, poisoned_model)
...
def test_activation_methods_stay_silent_on_clean_model(clean, clean_model):
    fs = TriggerArtifact().detect(clean, None, clean_model)
...
def test_ood_unavailable_without_reference(clean, clean_emb):
    fs = OutOfDistribution().detect(clean, clean_emb, None)
    assert len(fs) == 1 and fs[0].availability.value == "UNAVAILABLE" and "REFERENCE_CLEAN_SET" in fs[0].reason


def test_ood_clean_near_zero(clean, clean_emb, reference):
    rds, remb = reference
    fs = OutOfDistribution(rds, remb).detect(clean, clean_emb, None)
...
def test_ood_insertion_found(clean, reference, workdir):
    ...
    fs = OutOfDistribution(rds, remb).detect(ds, stub_embeddings(ds), None)
    flagged, inj = {f.target_ref for f in fs}, set(m["injected"])
    # Tolerant on purpose: a threshold estimated from a 200-image reference is seed-sensitive
    # (documented in NOTES); this asserts the mechanism, not a detection rate.
    assert len(flagged & inj) >= 0.8 * len(inj) and len(flagged - inj) <= 0.02 * len(clean)
    f = next(x for x in fs if x.target_ref in inj)
    assert f.attack_class == "out_of_distribution" and f.nature.value == "quality"
    assert f.severity.value == "medium"                            # contributor B is heavily OOD
    assert "leave-one-out" in f.reason and "'B'" in f.reason and f.score_raw > f.threshold

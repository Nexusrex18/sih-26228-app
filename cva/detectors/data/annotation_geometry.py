...
DEFAULTS = {"min_n": 15, "p_max": 1e-4, "effect_min": 1.0, "sample_z": 1.5,
...
class AnnotationGeometry:
    ...
    def detect(self, dataset: Dataset, embeddings=None, model=None) -> list:
        ...
        for cat in sorted(cats):
            usable = {c: np.stack([f for _, f in by[(c, cat)]]) for c in sorted(contribs)
                      if len(by.get((c, cat), [])) >= p["min_n"]}
            for c, M in usable.items():
                ...
                for j, name in enumerate(_FEATURES):
                    ...
                    if not (all(x >= p["effect_min"] for x in shifts) or all(x <= -p["effect_min"] for x in shifts)):
                        continue
                    if max(pv for _, _, pv in peers) >= p["p_max"]:
                        continue
                    ...
                    for (sid, _), zi in zip(mine, z):
                        if zi >= p["sample_z"]:
                            hits[sid].append({"contributor": c, "category": cat, "feature": name,
                                              ...
                                              "ks_p": pv, "n": len(mine), "peers": len(peers)})
        findings = []
        for sid, hs in sorted(hits.items()):
            ...
            lines = "; ".join(
                ...
                f"(KS p={h['ks_p']:.0e}, n={h['n']})" for f, h in feats.items())
            findings.append(make_finding(
                ...
                limitations=["No detector says where a box should be: 'anomalous' means unlike the same "

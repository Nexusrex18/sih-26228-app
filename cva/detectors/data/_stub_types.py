...
# Global-statistics descriptor (per-channel std, edge density, mean saturation), standardised by
# constants measured ONCE on the clean synthetic domain — a stand-in for a backbone trained
# in-domain, so out-of-domain imagery lands far away in it.
_STATS_MU = np.array([0.117, 0.107, 0.087, 0.038, 0.456], dtype=np.float32)
_STATS_SD = np.array([0.065, 0.060, 0.049, 0.008, 0.148], dtype=np.float32)


def _stats_block(a: np.ndarray) -> np.ndarray:
    mx, mn = a.max(axis=2), a.min(axis=2)
    sat = ((mx - mn) / np.maximum(mx, 1e-3)).mean()
    grad = np.abs(np.diff(a, axis=1)).mean() + np.abs(np.diff(a, axis=0)).mean()
    v = np.array([*a.reshape(-1, 3).std(axis=0), grad, sat], dtype=np.float32)
    return np.clip((v - _STATS_MU) / _STATS_SD, -20, 20)


def stub_embeddings(dataset: Dataset, dim: int = 64, size: int = 16, seed: int = 0,
                    semantic_weight: float = 0.8, stats_weight: float = 0.15) -> ArrayEmbeddingIndex:
    ...
    return ArrayEmbeddingIndex([s.sample_id for s in dataset.samples],
                               np.concatenate([Xc, semantic_weight * Sc, stats_weight * T], axis=1),
                               extractor_id="stub-semantic+pixel+stats", extractor_version="3")

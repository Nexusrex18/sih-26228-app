...
from cva.core.finding import Availability, Nature, Severity
from ._stub_types import Dataset
from .base import (EvidenceStore, Params, attribution, group_source, make_finding, not_performed,
                   register_data, to_model_input)
from .taxonomy import NEGATIVE_SPACE_POISONING

DEFAULTS = {"score_min": 0.8, "iou_max": 0.3, "rate_min": 0.15, "ratio_min": 3.0,
            "cohort_floor": 0.02, "p_max": 1e-3, "min_flagged": 5, "high_rate": 0.4, "batch": 32}
...
class NegativeSpace:
    ...
    requires = {Capability.DATASET_IMAGES, Capability.DATASET_LABELS,
                Capability.DATASET_CONTRIBUTOR_META, Capability.MODEL_PREDICT}
    ...
    def detect(self, dataset: Dataset, embeddings=None, model=None) -> list:
        ...
        for c in sorted(tot):
            k, n = hit[c], tot[c]
            rest_k, rest_n = sum(hit.values()) - k, sum(tot.values()) - n
            cohort = max(rest_k / rest_n if rest_n else 0.0, p["cohort_floor"])
            rate = k / n
            if k < p["min_flagged"] or rate < p["rate_min"] or rate < p["ratio_min"] * cohort:
                continue
            pv = float(binomtest(k, n, cohort, alternative="greater").pvalue)
            if pv >= p["p_max"]:
                continue
            ...
            for s in samples:
                ...
                findings.append(make_finding(
                    ...
                    reason=(f"The contributed model confidently detects an object "
                            ...
                            f"rest of cohort {cohort:.0%}, p={pv:.1e}) — a systematic pattern, not a "

...
class NegativeSpace:
    ...
    def _detect(self, dataset: Dataset, embeddings, model) -> list:
        ...
        if not samples:
            n_c = sum(1 for s in dataset.samples if s.contributor is not None)
            n_l = sum(1 for s in dataset.samples if s.labels)
            return [not_performed(self.id, self.version, self.attack_classes,
                                  f"no sample carries both a contributor and at least one annotation "
                                  f"({n_c} have a contributor, {n_l} have labels), so there is nothing "
                                  "to compare against a cohort")]

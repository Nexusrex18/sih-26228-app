...
class TriggerArtifact:
    ...
    def __init__(self, params: dict | None = None, evidence_dir=None,
                 candidate_samples: Sequence[str] | None = None):
        self.p = Params(DEFAULTS, params)
        self.ev = EvidenceStore(evidence_dir)
        self.candidates = list(candidate_samples) if candidate_samples else None
    ...
    def _saliency(self, dataset, model, fr, rec, notes, grid):
        ...
        if self.candidates:                                             # coarse sweep on named candidates
            ...
            if region is not None:
                groups[region].extend(x for x in self.candidates if x not in groups[region])
        for region, ids in groups.items():
            ...
            try:
                ...
                res = occlusion_test(model, X, region, grid, _control_regions(region, grid, p["control_regions"], p["seed"]))
    ...
    def _activation_methods(self, dataset, model, rec, notes):
        ...
        if not keep:
            return
        X = np.stack([to_model_input(s, model.input_shape) for s, _ in keep])
        y = np.array([d for _, d in keep])
        acts = None
        for i in range(0, len(X), 64):
            a = model.activations(X[i:i + 64])
            if not a:
                notes.append("activations not extractable: spectral/clustering not run")
                return
            names = list(a.keys())
            layer = names[-2] if len(names) >= 2 else names[-1]
            chunk = np.asarray(a[layer]).reshape(len(X[i:i + 64]), -1)
            acts = chunk if acts is None else np.concatenate([acts, chunk])
    ...
    def _finding(self, dataset, sid, ms, notes, grid, embeddings):
        ...
        sheet = None
        if self.ev.root is not None and "frequency_residue" in methods:
            blk = methods["frequency_residue"]["block"]
            mates = sorted(k for k, v in self._by_block.items() if v == blk)
            sheet = self.ev.png(contact_sheet(dataset, mates),
                                f"samples sharing the anomaly at block {blk}")
        return make_finding(
            ...
            limitations=[
                ...
                "Assumes category_id == the model's class index."] + notes)
...
class OutOfDistribution:
    ...
    def __init__(self, reference_set: Dataset | None = None,
                 reference_embeddings: EmbeddingIndex | None = None, params: dict | None = None,
                 evidence_dir=None, k: int = 5, quantile: float = 0.99, margin: float = 1.15,
                 min_reference: int = 50, contributor_share: float = 0.2, contributor_min: int = 10):
        self.ref, self.ref_emb = reference_set, reference_embeddings
        self.k, self.q, self.margin, self.min_ref = k, quantile, margin, min_reference
        self.share, self.share_min = contributor_share, contributor_min
        self.ev = EvidenceStore(evidence_dir)
        if params:
            raise ValueError("data.ood takes explicit keyword arguments, not a params dict")

    def detect(self, dataset: Dataset, embeddings: EmbeddingIndex | None = None, model=None) -> list:
        if self.ref is None or self.ref_emb is None:
            return [not_performed(self.id, self.version, self.attack_classes,
                                  "no REFERENCE_CLEAN_SET (with its embeddings) was supplied")]
        if embeddings is None:
            return [not_performed(self.id, self.version, self.attack_classes, "no embedding index supplied")]
        R = self.ref_emb.vectors([s.sample_id for s in self.ref.samples])
        if len(R) < self.min_ref:
            return [not_performed(self.id, self.version, self.attack_classes,
                                  f"reference set has {len(R)} images (< {self.min_ref}); too small for a stable threshold")]
        R = R / np.maximum(np.linalg.norm(R, axis=1, keepdims=True), 1e-12)
        k = min(self.k, len(R) - 1)
        S = R @ R.T
        np.fill_diagonal(S, -np.inf)
        d_ref = 1 - np.sort(S, axis=1)[:, -k:].mean(axis=1)
        tau = float(np.quantile(d_ref, self.q) * self.margin)
        ...
        for sid, dv in sorted(flagged.items()):
            s = dataset.sample(sid)
            c = s.contributor
            heavy = c is not None and per_c[c] >= self.share_min and per_c[c] / tot_c[c] >= self.share
            who = ""
            if c is not None:
